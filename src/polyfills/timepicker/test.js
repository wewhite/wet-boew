/**
 * @title Time Picker Unit Tests
 * @overview Test the time picker behaviour
 * @license wet-boew.github.io/wet-boew/License-en.html / wet-boew.github.io/wet-boew/Licence-fr.html
 * @author @LaurentGoderre
 */
( function( $, wb ) {

var runTest = Modernizr.inputtypes.time ? describe.skip : describe;

runTest( "Input type=\"time\" polyfill (time picker)", function() {
	var sandbox = sinon.sandbox.create(),
		...
		beforeFactory = function( elm, label ) {
			elm = elm || "<input type=\"time\" id=\"appointment\"/>";
			label = label || "<label for=\"appointment\">Appointment Time</label>";
	...
	before( function() {
		$document.on( "wb-init.wb-time", selector, function() {
	...
	describe( "initialization", function() {
		...
		it( "should have marked the element as initialized", function() {
			expect( $elm.hasClass( "wb-time-inited" ) ).to.equal( true );
		} );

		it( "should have created the time picker toggle button", function() {
	...
	describe( "with a time format and error in the label", function() {
		var label = "<label for=\"appointment\">" +
				"<span class=\"field-name\">Appointment Time</span>" +
	"<span class=\"timeicker-format\"> (<abbr title=\"TIME...Four digits year, dash, two digits month, dash, two digits day\">YYYY-MM-DD</abbr>)</span>" +
				"<strong class=\"error\" id=\"appointment-error\">" +
					"<span class=\"label label-danger\">" +
						"<span class=\"prefix\">Error 1: </span>Please enter a valid time" +
					"</span>" +
				"</strong>" +
			"</label>";
		...
		it( "should have stored only the field name in the label", function() {
			...
			expect( state.labelText ).to.equal( "Appointment Time" );
		} );
	} );

	describe( "with a populated datimete", function() {
		before( beforeFactory( "<input type=\"time\" id=\"appointment\" value=\"15:45\"/>" ) );
		after( defaultAfter );

		it( "should have stored the time in the state object", function() {
			var state = $elm.get( 0 ).state;
			expect( state.time.toString() ).to.equal( new Date( 2014, 7, 7, 15, 45, 59 ).toString() );
		} );
	} );

	describe( "toggle button", function() {
		var hiddenAltText = "Pick a time from a calendar for field: Appointment Time";
	...
	describe( "toggle button for disabled time field", function() {
		before( function( done ) {
			callback = function() {
				$elm.next().find( "a" ).click();
				done();
			};
			beforeFactory( "<input type=\"time\" readonly id=\"appointment\"/>" )();
		} );
		after( function() {
	...
	describe( "toggle button for read-only time field", function() {
		before( function( done ) {
			callback = function() {
				$elm.next().find( "a" ).click();
				done();
	...
	describe( "close button", function() {
		before( function( done ) {
			...
			beforeFactory( "<input type=\"time\" id=\"appointment\"/>" )();
		} );
		after( defaultAfter );
	...
	describe( "opening the calendar", function() {
		before( function( done ) {
			...
			beforeFactory( "<input type=\"time\" id=\"test\" min=\"2014-03-04\" max=\"2014-03-18\"/>", "<label for=\"test\">Test Time</label>" )();
		} );
		after( function() {
			$elm.next().find( "a" ).click();
			defaultAfter();

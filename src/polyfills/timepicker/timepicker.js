...
var componentName = "wb-time",
	...
	init = function( event ) {
		...
		if ( elm ) {
			...
			if ( !i18nText ) {
				...
				i18nText = {
					show: i18n( "time-show" ).replace( "\\'", "'" ) + space,
					hide: i18n( "time-hide" ).replace( "\\'", "'" ) + space +
						space + i18n( "esc-key" ).replace( "\\'", "'" ),
					selected: i18n( "time-sel" ).replace( "\\'", "'" )
				};
			}
	...
	createToggleIcon = function( elm ) {
		...
		$( "#" + wb.jqEscape( fieldId ) )
			.wrap( "<span class='wb-time-wrap input-group'></span>" )
			.after( objToggle );
		$container.slideUp( 0 );
	},
